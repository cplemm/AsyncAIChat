...
This repo contains a sample for implementing asynchronous, decoupled communication with a Large Language Model (LLM) for systems that need to scale to many concurrent users. For setup and deployment steps, go [getting started](#getting-started). For changes to the original sample, see [changes](./changes.md). For testing the solution, see [tests](./tests.md).
